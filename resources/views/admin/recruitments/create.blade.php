<x-layouts.admin>
    <h2 class="text-2xl font-semibold mb-6">Thêm tin tuyển dụng</h2>
    ...
    <form action="{{ route('admin.recruitments.store') }}" method="POST" class="space-y-4">
        @csrf
        ...
        <div>
            ...
            <textarea name="description" class="w-full border rounded px-3 py-2" rows="5">{{ old('description') }}</textarea>
        </div>
        ...
        <div>
            ...
            <input type="number" name="salary" value="{{ old('salary') }}" class="w-full border rounded px-3 py-2">
        </div>
